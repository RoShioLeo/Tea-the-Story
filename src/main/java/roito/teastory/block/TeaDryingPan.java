...
public class TeaDryingPan extends Block
{
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        if (this.getMetaFromState(state) == 0)
        {
            ...
            if (worldIn.isRemote && ConfigMain.general.blockMessage && !playerIn.getHeldItem(hand).isEmpty() && playerIn.getHeldItem(hand).getItem() == ItemRegister.washed_rice && playerIn.getHeldItem(hand).getCount() < ConfigMain.others.cookRiceBallEachTime * 8)
            {
                playerIn.sendMessage(new TextComponentTranslation("teastory.message.cooking_pan.notenough"));
                return true;
            }
            else if (worldIn.isRemote && ConfigMain.general.blockMessage && (playerIn.getHeldItem(hand).isEmpty() || Block.getBlockFromItem(playerIn.getHeldItem(hand).getItem()) != BlockRegister.tea_drying_pan))
            {
        ...
        else if (this.getMetaFromState(state) == 1)
        {
            ...
            if (worldIn.isRemote && ConfigMain.general.blockMessage)
            {
