...
public class AqueductBlock extends HorizontalConnectedBlock implements IWaterLoggable
{
    ...
    {
        return !(state.getBlock() instanceof PaddyFieldBlock) && (state.isAir() || state.getBlock() instanceof AqueductBlock || (state.getFluidState().getFluid() == Fluids.WATER || state.getFluidState().getFluid() == Fluids.FLOWING_WATER));
    }
